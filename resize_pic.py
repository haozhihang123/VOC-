...
# 按指定图像大小调整尺寸
def resize_image(image, height = 500, width = 500):
...
def read__image(path_name,result_path):
  num = 0
  for dir_image in os.listdir(path_name): # os.listdir() 方法用于返回指定的文件夹包含的文件或文件夹的名字的列表
    ...
    if os.path.isdir(full_path): #如果是文件夹，继续递归调用
      ...
      if dir_image.endswith('.jpg'):
        ...
        print(image_name)
        cv2.imwrite(result_path + image_name, image)
        num = num + 1

if __name__=='__main__':
    #存储路径
    result_path = './result/'
    y = os.path.exists(result_path)
    if y != 1:
        os.mkdir(result_path)
    read__image('JPEGImages',result_path)
